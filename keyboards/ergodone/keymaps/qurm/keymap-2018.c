...
//AF Custom definitions
#define Pwd 30 // -> macro
#define UID1 31 // -> macro
#define UID2 32 // -> macro
...
//,KC_LCTL   ,MEH_T(KC_NO)   ,MT(MOD_HYPR,KC_CUT),MT(MOD_LALT,KC_COPY), MT(MOD_LGUI,KC_PASTE)
                                         ,KC_HOME,KC_END
                                                 ,KC_PGUP
...
,KC_NO ,KC_NO   ,KC_BTN3 ,KC_BTN1  ,KC_BTN2
  ...
  //                                                                   ,KC_NO    ,KC_F16  ,KC_F17       ,KC_F18       ,KC_F19  ,KC_F20  ,KC_TRNS
                                                                     ,RESET    ,KC_F16  ,KC_F17       ,KC_F18       ,KC_F19  ,KC_F20  ,KC_TRNS
...
[SPEC] = LAYOUT_ergodox(
// left hand
 KC_TRNS ,KC_ESC  ,KC_TRNS ,KC_TRNS ,M(UID1) ,M(UID2) ,M(Pwd)
 ...
 *                                  `--------------------'       `--------------------'
 */
// AF curently not used - disabled as triggered when not wanted
...
const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt)
{
    switch(id) {
        ...
        break;
/* 
AF 15/7/2018  Custom section
Macro Commands - uses older style as the rest of the keymap uses these.
...
*/
		...
		case UID1:
        if (record->event.pressed) {
            return MACRO( I(5), T(U), T(I), T(D), T(1), END  ); // ->
        }
        break;
